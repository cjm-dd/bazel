 ...
 * is just the predeclared inputs hash. Otherwise, the AC entry for the predeclared inputs hash will
 * be an intermediate entry that lists one or more sets of {@link RepoRecordedInput}s that a
 * previously cached repo consumed during the evaluation of its rule. The cache requests the current
 * values of these inputs and computes the next hash to look up by a rolling construction that
 * combines the previous hash with the string representations of the {@link
 * RepoRecordedInput.WithValue}. This process is repeated until a final entry with the repo contents
 * is found or no matching entry exists.
 *
...
public final class RemoteRepoContentsCacheImpl implements RemoteRepoContentsCache {
  ...
  public void addToCache(
    ...
    // The repo contents cache only stores results in the remote cache when its file system overlay
    // is installed, which only happens with --experimental_remote_repo_contents_cache. This matches
    // the check in lookupCache and ensures a plain remote build does not pollute the action cache
    // with repository entries.
  ...
  private boolean doLookupCache(
    ...
    var maybeRecordedInputs = readMarkerFile(markerFileContent, predeclaredInputHash);
    ...
    return Futures.transformAsync(
        ...
        currentInputsString -> {
          ...
          return whenAllSucceed(
                  cache.uploadBlob(context, actionKey.getDigest(), action.toByteString()),
  ...
  private static Optional<ImmutableList<RepoRecordedInput.WithValue>> readMarkerFile(
    ...
    for (String line : Splitter.on('\n').split(content)) {
      ...
      if (!firstLineVerified) {
        ...
        Optional<RepoRecordedInput.WithValue> withValue = RepoRecordedInput.WithValue.parse(line);
        if (withValue.isEmpty()) {
          return Optional.empty();
        }
        recordedInputValues.add(withValue.get());
      }
    }
    if (!firstLineVerified) {
      return Optional.empty();
